...
def check(start_station,end_station,date,TrainNo):
    with open('Data_Seat_Check.json','r') as f:
        seat_data_check = json.load(f)
        for i in seat_data_check["AvailableSeats"]:
            if i["OriginStationName"]["Zh_tw"] == start_station and i["DestinationStationName"]["Zh_tw"] == end_station:
                TrainNo.append(i["TrainNo"])
                
                
def check_seat(start_station, end_station,date,Train_No):
    with open('Data_Seat_Check.json','r') as f:
        seat_data = json.load(f)   
        for i in seat_data["AvailableSeats"]:
            if i["TrainNo"] == Train_No:
                if i["OriginStationName"]["Zh_tw"] == start_station and i["DestinationStationName"]["Zh_tw"] == end_station:
                    if i["StandardSeatStatus"] == "O" or i["StandardSeatStatus"] == "L":
